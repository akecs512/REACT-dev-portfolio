
export const travelogueItems = {
  screenshot: "/images/Travelogue.png",
  screenshotAlt: "travelogue",
  icons: [
    { path: "/images/JSLogo.png", alt: "JS" },
    { path: "/images/handlebarsLogo.png", alt: "handlebars" },
    { path: "/images/icons/skills/nodejswhite.png", alt: "nodejs" },
    { path: "/images/mysqlLogo.jpg", alt: "mySql" },
    { path: "/images/sequelizeLogo.png", alt: "Sequelize" },
  ],
  pageLink: "https://travelogue-application-10ca673e5794.herokuapp.com/",
  repoLink: "https://github.com/akecs512/Travelogue-application",
  tags: ["Node.js", "JavaScript", "Handlebars.js", "MySQL", "Sequelize"],
  bodyText: [
    "Travelers log into to save destinations, dates, and notes about their travels",
    "Collaborative project",
  ],
};

export const wodItems = {
  screenshot: "/images/wod.png",
  screenshotAlt: "WOD",
  icons: [
    { path: "/images/JSLogo.png", alt: "JS" },
    { path: "/images/icons/skills/html.png", alt: "html" },
    { path: "/images/icons/skills/css.png", alt: "css" },
    {
      path: "/images/icons/skills/materialize.png",
      alt: "materialize",
    },
    { path: "/images/spoonLogo.png", alt: "spoonacular" },
    { path: "/images/tmdb.png", alt: "movieDB" },
  ],
  pageLink: "https://esbev.github.io/project-1-collab/",
  repoLink: "https://github.com/akecs512/Wheels-of-Date-Night",
  tags: [
    "HTML",
    "CSS",
    "JavaScript",
    "Materialize",
    "Spoonacular API",
    "TMDB API",
  ],
  bodyText: [
    "Users can search for a dish and movie combination for their next date night using spinning wheels",
    "Collaborative project",
  ],
};

export const techblogItems = {
  screenshot: "/images/techblog01.png",
  screenshotAlt: "techblog",
  icons: [
    { path: "/images/JSLogo.png", alt: "JS" },
    { path: "/images/handlebarsLogo.png", alt: "handlebars" },
    { path: "/images/icons/skills/nodejswhite.png", alt: "nodejs" },
    { path: "/images/mysqlLogo.jpg", alt: "mySql" },
    { path: "/images/sequelizeLogo.png", alt: "Sequelize" },
  ],
  pageLink: "https://tech-blog-app-mvc-446a1fb32162.herokuapp.com/",
  repoLink: "https://github.com/akecs512/MVC-tech-blog",
  tags: ["Node.js", "Express.js", "Handlebars.js", "MySQL", "Sequelize"],
  bodyText: [
    "Users can create an account, post, edit, and comment on posts",
    "MVC paradigm",
  ],
};
export const weatherItems = {
  screenshot: "/images/Weather.png",
  screenshotAlt: "weather dashboard",
  repoLink: "https://github.com/akecs512/SERVER-SIDE-API-weather-dashboard",
  pageLink: "https://akecs512.github.io/SERVER-SIDE-API-weather-dashboard/",
  tags: ["HTML", "CSS", "JavaScript", "OpenWeather API"],
  bodyText: [
    "A weather dashboard that displays the current weather and 5 day forecast for a searched city",
    "Uses the OpenWeather API",
  ],
};
export const noteItems = {
  screenshot: "/images/noteTaker.png",
  screenshotAlt: "Note Taker",
  bodyText: [
    "A note taking app that saves notes to a JSON file",
    "View, save, and delete notes",
  ],
  tags: ["HTML", "CSS", "JavaScript", "Express.js", "Node.js", "Day.js"],
  repoLink: "https://github.com/akecs512/EXPRESS-JS-note-taker",
  pageLink: "https://adriennes-note-taker-app-3061299dc485.herokuapp.com/",
};

export const projectItems = {
  screenshot: "/images/gitFit.png",
  screenshotAlt: "Git Fit",
  bodyText: ["A fitness tracker that allows users to track their workouts", "Includes a dashboard that displays workout data in a chart and a Bmi calculator", "Collaborative project"],
  tags: ["React", "Express.js", "Node.js", "MongoDB", "Mongoose", "Chart.js", "Tailwind CSS", "DaisyUI", "Heroku"],
  repoLink: "https://github.com/akecs512/REACT-dev-portfolio",
  pageLink:"https://akecs-dev-portfolio.netlify.app/",

};